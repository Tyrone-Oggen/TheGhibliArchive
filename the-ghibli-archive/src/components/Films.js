...
const ghibliData = axios.get("https://ghibliapi.herokuapp.com/films");

const MovieList = props => (
  <div className="movie-list">
    {props.films.map(film => (
      <GhibliMovie key={film.id} {...film} />
    ))}
  </div>
);

class GhibliMovie extends React.Component {
  render() {
    ...
    return (
      <div className="ghibli-movie">
        <div className="movie-poster">
          <img src={avatar_url} alt={film.title} />
        </div>
        <div className="movie-info">
          <div className="movie-name">{film.title}</div>
          {/* <div className="movie-description">{film.description}</div> */}
        </div>
      </div>
    );
  }
